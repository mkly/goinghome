...
// Someone who has asked for less motion still gets the ball, just parked - and
// scrolling does not kick it either.
...
function flood(energy, size, color, pos) {
  const l = new THREE.RectAreaLight(new THREE.Color(...color), energy * LIGHT_SCALE, size, size);
...
const keyLight   = flood(33.0, 5.7, [1.00, 0.72, 0.41], [ 0.80,  1.05,  0.40]);
const rimLight   = flood(30.0, 5.1, [1.00, 0.63, 0.33], [-0.45,  0.75, -1.10]);
const fillLight  = flood(2.6, 15.7, [1.00, 0.66, 0.38], [-0.85,  0.20,  0.50]);
const underLight = flood(0.9, 15.7, [0.70, 0.79, 1.00], [ 0.10, -0.90, -0.20]);
...
function integrateFling(dt) {
  ...
  if (flyVec.length() < 0.01) { flyVec.set(0, 0, 0); return; }
  spinAbout(flyVec, dt);
}
...
let tipA = 0, tipB = 0, lastKickT = -1e9;
...
function scrollKick(dpx) {
  ...
  _kick.copy(_right).multiplyScalar(mag)
...
function randomSpin() {
  ...
  _kick.copy(_right).multiplyScalar(Math.cos(a))
...
try {
  ...
  host.addEventListener('scroll', e => {   // deltas and fake a huge jump
    const el = e.target === host ? host.documentElement : e.target;
    const top = el.scrollTop;
    if (seen.has(el)) scrollKick(top - seen.get(el));
    seen.set(el, top);
  }, true);
} catch { /* cross-origin host: no scroll input, the loop still runs */ }
...
new GLTFLoader().load('./baseball.glb', gltf => {
  ...
  const sphere = new THREE.Box3().setFromObject(ball).getBoundingSphere(new THREE.Sphere());
